...
# ── Vendored schema mirror integrity ────────────────────────────────
#
# tests/schemas/v1/ is the schema source of record on a standalone clone: the
# mirror ships in this repo and scripts/generate-protocol.py falls back to it
# when no sibling protocol checkout exists.
#
# tests/schemas/manifest.json maps every vendored filename to its sha256. It is
# REGENERATED by the refresh step — scripts/refresh-vendored-schemas.sh, which
# copies get-h3/protocol schemas/v1/*.json into the mirror and rewrites the
# manifest — and it is the reference these tests compare against, so they hold
# on a standalone clone with NO sibling checkout.
#
# Both tests FAIL (they never skip) when a vendored schema is missing, extra, or
# byte-different from its manifest entry: a stale mirror must not re-ship.
#
# NOTE: the subject here is the VENDORED mirror, not SCHEMA_DIR. H3_SCHEMA_DIR
# deliberately points the validation tests at a *newer* protocol checkout,
# while the mirror must stay self-consistent wherever the suite runs.

VENDORED_SCHEMA_DIR = Path(__file__).resolve().parent / "schemas" / "v1"
VENDORED_MANIFEST = Path(__file__).resolve().parent / "schemas" / "manifest.json"

# The v1 schema set the mirror vendors. Explicit so a shrunken mirror cannot
# agree with a manifest regenerated from that same shrunken directory: growing
# the protocol set means running the refresh step AND bumping this constant.
VENDORED_SCHEMA_COUNT = 17


def _vendored_schema_files() -> list[str]:
    assert VENDORED_SCHEMA_DIR.is_dir(), (
        f"Vendored schema mirror missing: {VENDORED_SCHEMA_DIR}"
    )
    return sorted(p.name for p in VENDORED_SCHEMA_DIR.glob("*.json"))


def _vendored_manifest() -> dict[str, str]:
    assert VENDORED_MANIFEST.is_file(), (
        f"Vendored schema manifest missing: {VENDORED_MANIFEST} — regenerate it "
        "with scripts/refresh-vendored-schemas.sh"
    )
    manifest = json.loads(VENDORED_MANIFEST.read_text())
    assert isinstance(manifest, dict) and manifest, (
        f"{VENDORED_MANIFEST} must be a non-empty object of filename -> sha256"
    )
    return manifest
...
def test_vendored_schema_files_match_manifest_keys():
    """The mirror's *.json set equals the manifest's key set (count + names)."""
    manifest = _vendored_manifest()
    files = _vendored_schema_files()

    assert len(files) == VENDORED_SCHEMA_COUNT, (
        f"Vendored schema mirror holds {len(files)} *.json files, expected "
        f"{VENDORED_SCHEMA_COUNT}: {files} — refresh the mirror and manifest "
        "with scripts/refresh-vendored-schemas.sh"
    )
    assert set(files) == set(manifest), (
        "Vendored schema mirror and manifest disagree: "
        f"only on disk={sorted(set(files) - set(manifest))}, "
        f"only in manifest={sorted(set(manifest) - set(files))} — refresh the "
        "mirror and manifest with scripts/refresh-vendored-schemas.sh"
    )
    assert len(manifest) == VENDORED_SCHEMA_COUNT


def test_vendored_schema_hashes_match_manifest():
    """Every vendored schema's sha256 equals its manifest entry."""
    manifest = _vendored_manifest()

    missing = [n for n in sorted(manifest) if not (VENDORED_SCHEMA_DIR / n).is_file()]
    mismatched = [
        f"{n}: on disk {_sha256(VENDORED_SCHEMA_DIR / n)}, manifest {manifest[n]}"
        for n in sorted(manifest)
        if (VENDORED_SCHEMA_DIR / n).is_file()
        and _sha256(VENDORED_SCHEMA_DIR / n) != manifest[n]
    ]

    assert not missing, (
        f"Vendored schemas listed in the manifest but absent: {missing} — "
        "refresh the mirror with scripts/refresh-vendored-schemas.sh"
    )
    assert not mismatched, (
        "Vendored schemas differ from the manifest:\n  " + "\n  ".join(mismatched)
    )
